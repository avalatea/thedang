...
from thedang import const


@pytest.fixture
def load_source(mocker):
    return mocker.patch('thedang.conf.load_source')
...
class TestSettingsFromEnv(object):
    def test_from_env(self, os_environ, settings):
        os_environ.update({'THEDANG_RULES': 'bash:lisp',
                           'THEDANG_EXCLUDE_RULES': 'git:vim',
                           'THEDANG_WAIT_COMMAND': '55',
                           'THEDANG_REQUIRE_CONFIRMATION': 'true',
                           'THEDANG_NO_COLORS': 'false',
                           'THEDANG_PRIORITY': 'bash=10:lisp=wrong:vim=15',
                           'THEDANG_WAIT_SLOW_COMMAND': '999',
                           'THEDANG_SLOW_COMMANDS': 'lein:react-native:./gradlew',
                           'THEDANG_NUM_CLOSE_MATCHES': '359',
                           'THEDANG_EXCLUDED_SEARCH_PATH_PREFIXES': '/media/:/mnt/'})
        settings.init()
    ...
    def test_from_env_with_DEFAULT(self, os_environ, settings):
        os_environ.update({'THEDANG_RULES': 'DEFAULT_RULES:bash:lisp'})
        settings.init()
...
@pytest.mark.parametrize('legacy_dir_exists, xdg_config_home, result', [
    (False, '~/.config', '~/.config/thedang'),
    (False, '/user/test/config/', '/user/test/config/thedang'),
    (True, '~/.config', '~/.thedang'),
    (True, '/user/test/config/', '~/.thedang')])
def test_get_user_dir_path(mocker, os_environ, settings, legacy_dir_exists,
                           xdg_config_home, result):
    mocker.patch('thedang.conf.Path.is_dir',
