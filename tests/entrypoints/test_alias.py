...
import pytest
from thedang.entrypoints.alias import _get_alias, print_alias
...
def test_get_alias(monkeypatch, mocker, py2,
    ...
    args = Mock(
        ...
        alias='dang', )
    mocker.patch('thedang.entrypoints.alias.which', return_value=which)
    shell = Mock(app_alias=lambda _: 'app_alias',
                 instant_mode_alias=lambda _: 'instant_mode_alias')
    monkeypatch.setattr('thedang.entrypoints.alias.shell', shell)
...
def test_print_alias(mocker):
    settings_mock = mocker.patch('thedang.entrypoints.alias.settings')
    _get_alias_mock = mocker.patch('thedang.entrypoints.alias._get_alias')
    known_args = Mock()
