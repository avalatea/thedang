...
from thedang.utils import default_settings, \
...
class TestGetCloseMatches(object):
    @patch('thedang.utils.difflib_get_close_matches')
...
def get_aliases(mocker):
    mocker.patch('thedang.shells.shell.get_aliases',
                 return_value=['vim', 'apt-get', 'fsck', 'dang'])
...
def test_get_all_executables():
    ...
    assert 'dang' not in all_callables
...
def test_get_all_executables_pathsep(path, pathsep):
    with patch('thedang.utils.Path') as Path_mock:
        get_all_executables()
...
class TestCache(object):
    ...
    def shelve(self, mocker):
        ...
        mocker.patch('thedang.utils.shelve.open', new_callable=lambda: _Shelve)
        return value

    @pytest.fixture(autouse=True)
    def enable_cache(self, monkeypatch, shelve):
        monkeypatch.setattr('thedang.utils.cache.disabled', False)
        _cache._init_db()

    @pytest.fixture(autouse=True)
    def mtime(self, mocker):
        mocker.patch('thedang.utils.os.path.getmtime', return_value=0)
    ...
    def key(self, monkeypatch):
        monkeypatch.setattr('thedang.utils.Cache._get_key',
                            lambda *_: 'key')
        return 'key'
...
class TestGetValidHistoryWithoutCurrent(object):
    ...
    def history(self, mocker):
        mock = mocker.patch('thedang.shells.shell.get_history')
        #  Passing as an argument causes `UnicodeDecodeError`
        #  with newer pytest and python 2.7
        mock.return_value = ['le cat', 'dang', 'ls cat',
                             'diff x', 'nocommand x', u'café ô']
        return mock

    @pytest.fixture(autouse=True)
    def alias(self, mocker):
        return mocker.patch('thedang.utils.get_alias',
                            return_value='dang')
    ...
    def bins(self, mocker):
        ...
        return mocker.patch('thedang.utils.Path', return_value=path_mock)

    @pytest.mark.parametrize('script, result', [
        ('le cat', ['ls cat', 'diff x', u'café ô']),
        ('diff x', ['ls cat', u'café ô']),
        ('dang', ['ls cat', 'diff x', u'café ô']),
