...
import os
import pytest
from thedang.shells import Bash
...
class TestBash(object):
    ...
    def Popen(self, mocker):
        mock = mocker.patch('thedang.shells.bash.Popen')
        return mock

    @pytest.fixture(autouse=True)
    def shell_aliases(self):
        os.environ['TF_SHELL_ALIASES'] = (
            'alias dang=\'eval $(thedang $(fc -ln -1))\'\n'
            'alias l=\'ls -CF\'\n'
            'alias la=\'ls -A\'\n'
            'alias ll=\'ls -alF\'')
    ...
    def test_get_aliases(self, shell):
        assert shell.get_aliases() == {'dang': 'eval $(thedang $(fc -ln -1))',
                                       'l': 'ls -CF',
                                       'la': 'ls -A',
                                       'll': 'ls -alF'}

    def test_app_alias(self, shell):
        assert 'dang () {' in shell.app_alias('dang')
        assert 'DANG () {' in shell.app_alias('DANG')
        assert 'thedang' in shell.app_alias('dang')
        assert 'PYTHONIOENCODING' in shell.app_alias('dang')

    def test_app_alias_variables_correctly_set(self, shell):
        alias = shell.app_alias('dang')
        assert "dang () {" in alias
        assert 'TF_SHELL=bash' in alias
        assert "TF_ALIAS=dang" in alias
