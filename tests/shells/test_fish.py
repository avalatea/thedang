...
from thedang.shells import Fish
...
class TestFish(object):
    ...
    def Popen(self, mocker):
        mock = mocker.patch('thedang.shells.fish.Popen')
    ...
    @pytest.mark.parametrize('key, value', [
        ...
        ('THEDANG_OVERRIDDEN_ALIASES', 'cut,git,sed'),
        ('THEDANG_OVERRIDDEN_ALIASES', 'cut, git, sed'),
        ('THEDANG_OVERRIDDEN_ALIASES', ' cut,\tgit,sed\n'),
        ('THEDANG_OVERRIDDEN_ALIASES', '\ncut,\n\ngit,\tsed\r')])
    ...
    def test_app_alias(self, shell):
        ...
        assert 'TF_SHELL=fish' in shell.app_alias('dang')
        assert 'TF_ALIAS=dang PYTHONIOENCODING' in shell.app_alias('dang')
        assert 'PYTHONIOENCODING=utf-8 thedang' in shell.app_alias('dang')
        assert ARGUMENT_PLACEHOLDER in shell.app_alias('dang')

    def test_app_alias_alter_history(self, settings, shell):
        settings.alter_history = True
        assert (
            'builtin history delete --exact --case-sensitive -- $danged_up_command\n'
            in shell.app_alias('DANG')
        )
        assert 'builtin history merge\n' in shell.app_alias('DANG')
        settings.alter_history = False
        assert 'builtin history delete' not in shell.app_alias('DANG')
        assert 'builtin history merge' not in shell.app_alias('DANG')
    ...
    def test_put_to_history(self, entry, entry_utf8, builtins_open, mocker, shell):
        mocker.patch('thedang.shells.fish.time', return_value=1430707243.3517463)
