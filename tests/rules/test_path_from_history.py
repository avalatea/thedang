import pytest
from thedang.rules.path_from_history import match, get_new_command
...
def history(mocker):
    return mocker.patch(
        'thedang.rules.path_from_history.get_valid_history_without_current',
        return_value=['cd /opt/java', 'ls ~/work/project/'])


@pytest.fixture(autouse=True)
def path_exists(mocker):
    path_mock = mocker.patch('thedang.rules.path_from_history.Path')
