import pytest
from thedang.rules.scm_correction import match, get_new_command
from thedang.types import Command


@pytest.fixture
def get_actual_scm_mock(mocker):
    return mocker.patch('thedang.rules.scm_correction._get_actual_scm',
                        return_value=None)
...
@pytest.mark.parametrize('script, output, actual_scm', [
    ('git log', 'fatal: Not a git repository '
     ...
     'hg'),
    ('hg log', "abort: no repository found in '/home/nvbn/exp/thedang' "
               "(.hg not found)!",
     'git')])
...
@pytest.mark.parametrize('script, output, actual_scm', [
    ...
    ('hg log', "abort: no repository found in '/home/nvbn/exp/thedang' "
               "(.hg not found)!",
     None),
    ('not-scm log', "abort: no repository found in '/home/nvbn/exp/thedang' "
                    "(.hg not found)!",
     'git')])
