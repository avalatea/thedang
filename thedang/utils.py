...
class Cache(object):
    ...
    def _setup_db(self):
        ...
        cache_path = Path(cache_dir).joinpath('thedang').as_posix()
...
def get_installation_version():
    try:
        ...
        return version('thedang')
    except ImportError:
        import pkg_resources

        return pkg_resources.require('thedang')[0].version


def get_alias():
    return os.environ.get('TF_ALIAS', 'dang')


@memoize
def get_valid_history_without_current(command):
    def _not_corrected(history, tf_alias):
        """Returns all lines from history except that comes before `dang`."""
        previous = None
        for line in history:
